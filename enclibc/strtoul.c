// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

#include <ctype.h>
#include <limits.h>
#include <openenclave/enclave.h>
#include <openenclave/internal/enclavelibc.h>
#include <stdint.h>
#include <string.h>

//
// If c is a digit character:
//     then: _digit[c] yields the integer value for that digit character.
//     else: _digit[c] yields 0xFF.
//
// Digit characters fall within these ranges: ['0'-'9'] and ['A'-'Z'].
//
// Examples:
//     _digit['9'] => 9
//     _digit['A'] => 10
//     _digit['Z'] => 35
//     _digit['?'] => 0xFF
//
static const unsigned char _digit[256] = {
    ,
};

/* Return true if c is a digit character within the given base */
ENCLIBC_INLINE bool _isdigit(char c, int base)
{
    return _digit[(unsigned char)c] < base;
}

unsigned long int enclibc_strtoul(const char* nptr, char** endptr, int base)
{
    const char* p;
    unsigned long x = 0;
    bool negative = false;

    if (endptr)
        *endptr = (char*)nptr;

    if (!nptr)
        return 0;

    /* Set scanning pointer to nptr */
    p = nptr;

    /* Skip any leading whitespace */
    while (isspace(*p))
        p++;

    /* Handle '+' and '-' */
    if (p[0] == '+')
    {
        p++;
    }
    else if (p[0] == '-')
    {
        negative = true;
        p++;
    }

    /* Handle case where base == 0 */
    if (base == 0)
    {
        if (p[0] == '0' && (p[1] == 'x' || p[1] == 'X'))
        {
            base = 16;
            p += 2;
        }
        else if (p[0] == '0')
        {
            base = 8;
            p++;
        }
        else
        {
            base = 10;
        }
    }

    for (; *p && _isdigit(*p, base); p++)
    {
        /* Multiply by base */
        {
            /* Check for overflow */
            if (x > ENCLIBC_UINT64_MAX / base)
            {
                if (endptr)
                    *endptr = (char*)p;

                return ENCLIBC_UINT64_MAX;
            }

            x = x * base;
        }

        /* Add digit */
        {
            const unsigned long digit = _digit[(unsigned char)*p];

            /* Check for overflow */
            if (digit > ENCLIBC_ULONG_MAX - x)
            {
                if (endptr)
                    *endptr = (char*)p;

                return ENCLIBC_UINT64_MAX;
            }

            x += digit;
        }
    }

    /* Return zero if no digits were found */
    if (p == nptr)
        return 0;

    if (endptr)
        *endptr = (char*)p;

    /* Invert if negative */
    if (negative)
    {
        x = -(int64_t)x;
    }

    return x;
}

/*
**==============================================================================
**
** Use the following functions to generate the _digit[] table above.
**
**==============================================================================
*/

#if defined(ENCLIBC_NEED_STRTOUL_GENERATOR)

void __enclibc_gen_strtoul_table(void)
{
    for (int i = 0; i < 256; i++)
    {
        if (i >= '0' && i <= '9')
        {
            enclibc_printf("0x%02x,\n", i - '0');
        }
        else if (i >= 'A' && i <= 'Z')
        {
            enclibc_printf("0x%02x,\n", i - 'A' + 10);
        }
        else if (i >= 'a' && i <= 'z')
        {
            enclibc_printf("0x%02x,\n", i - 'a' + 10);
        }
        else
        {
            enclibc_printf("0xFF,\n");
        }
    }
}

#endif /* defined(ENCLIBC_NEED_STRTOUL_GENERATOR) */

...
#include <openenclave/iot/stdiobegin.h>
...
#define oe_fopen(file_security, path, mode) \
    OE_PRIMITIVE_CAT(oe_fopen_, file_security((path), (mode)))
#define oe_remove(file_security, pathname) \
    OE_PRIMITIVE_CAT(oe_remove_, file_security(pathname))
...
#ifdef OE_SECURE_POSIX_FILE_API
#define opendir(name) oe_opendir(OE_FILE_SECURE_BEST_EFFORT, name)
#elif !defined(OE_NO_POSIX_FILE_API)
#define opendir(name) oe_opendir(OE_FILE_INSECURE, name)
#endif
...
#include <openenclave/iot/stdioend.h>

    ...
    bool* flag)
{
    ...
    {
        bool tmp;
        OE_CHECK(oe_struct_eq(sti, s1, s2, &tmp));
        if (!tmp)
            OE_RAISE(OE_OK);
    }
    ...
    bool* flag)
{
    ...
    if (fti->flags & OE_FLAG_COUNT)
    {
        OE_CHECK(_GetCount(sti, s1, fti, &count1));
        OE_CHECK(_GetCount(sti, s2, fti, &count2));
    }
    ...
    if (fti->flags & OE_FLAG_PTR)
    {
        ...
        else if (fti->type == OE_STRUCT_T)
        {
            ...
            OE_CHECK(_StructsEq(fti->sti, p1, p2, count1, &tmp));
    ...
    else if (fti->flags & OE_FLAG_ARRAY)
    {
        ...
        else if (fti->type == OE_STRUCT_T)
        {
            bool tmp;
            OE_CHECK(
                _StructsEq(fti->sti, f1, f2, (size_t)fti->subscript, &tmp));
            if (!tmp)
                OE_RAISE(OE_OK);
        }
        else
        {
            if (!_ArrayEq(fti->type, f1, f2, (size_t)fti->subscript))
                OE_RAISE(OE_OK);
        }
    }
    else
    {
        ...
        {
            bool tmp;
            OE_CHECK(_StructsEq(fti->sti, f1, f2, 1, &tmp));
            if (!tmp)
                OE_RAISE(OE_OK);
        }
        else
    ...
    bool* flag)
{
    ...
    {
        bool tmp;
        OE_CHECK(_FieldEq(sti, s1, s2, i, &tmp));
        if (!tmp)
            OE_RAISE(OE_OK);
    }
    ...
    for (i = 0; i < ti->nfields; i++)
    {
        ...
        else if (fti->flags & OE_FLAG_ARRAY)
        {
            _PrintArray(
                fti,
                p,
                fti->size / (size_t)fti->subscript,
                (size_t)fti->subscript,
                depth);
            OE_PRINTF("\n");
        }
        else
    ...
    void*(alloc)(size_t size))
{
    ...
    OE_CHECK(_GetCount(sti, sin, fti, &count));
    ...
    else if (fti->flags & OE_FLAG_COUNT)
    {
        ...
        if (fti->type == OE_STRUCT_T)
        {
            OE_CHECK(_CopyStructs(fti->sti, ptrIn, count, *ptrOut, alloc));
        }
        else
        {
    ...
    void*(alloc)(size_t size))
{
    ...
    else if (fti->flags & OE_FLAG_PTR)
    {
        ...
        OE_CHECK(_ClonePtrField(sti, sin, fti, finp, &foutp, alloc));
    ...
    {
        if (fti->type == OE_STRUCT_T)
            OE_CHECK(_CopyStructs(
                fti->sti, fin, (size_t)fti->subscript, fout, alloc));
        else
            memcpy(fout, fin, fti->size);
    }
    else /* scalar */
    {
        if (fti->type == OE_STRUCT_T)
            OE_CHECK(oe_copy_struct(fti->sti, fin, fout, alloc));
        else
            memcpy(fout, fin, fti->size);
    }
    ...
    oe_dealloc_proc_t dealloc)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        OE_CHECK(oe_destroy_struct(sti, p, dealloc));
        p += sti->size;
    }
    ...
    void*(alloc)(size_t size))
{
    ...
    for (i = 0; i < sti->nfields; i++)
    {
        ...
        OE_CHECK(_CopyField(sti, sin, fti, fin, fout, alloc));
    }
    ...
    void*(alloc)(size_t size))
{
    ...
    OE_CHECK(_CopyStructs(sti, sin, count, *sout, alloc));
    ...
    void* procData)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        OE_CHECK(_ApplyStructPtrProc(ti, s, proc, procData));
    }
    ...
    for (i = 0; i < ti->nfields; i++)
    {
        ...
        else if (fti->flags & OE_FLAG_PTR)
        {
            ...
            else if (fti->flags & OE_FLAG_COUNT)
            {
                ...
                OE_CHECK(_GetCount(ti, strct, fti, &count));

                /* Handle struct-pointers with [count] qualifier */
                if (fti->type == OE_STRUCT_T)
                {
                    OE_CHECK(_ApplyStructsPtrProc(
        ...
        else if (fti->flags & OE_FLAG_ARRAY)
        {
            ...
            {
                OE_CHECK(_ApplyStructsPtrProc(
                    fti->sti, fptr, (size_t)fti->subscript, proc, procData));
            }
        }
        else /* scalar */
        {
            if (fti->type == OE_STRUCT_T)
            {
                OE_CHECK(_ApplyStructPtrProc(fti->sti, fptr, proc, procData));
            }
        }
    }
    ...
    oe_dealloc_proc_t dealloc)
{
    ...
    OE_CHECK(_ApplyStructPtrProc(ti, strct, _FreeProc, dealloc));
    ...
    oe_dealloc_proc_t dealloc)
{
    ...
    OE_CHECK(oe_destroy_struct(ti, strct, dealloc));
    dealloc(strct);
    ...
    oe_dealloc_proc_t dealloc)
{
        ...
        else if (fti->flags & OE_FLAG_COUNT)
        {
            if (fti->type == OE_STRUCT_T)
            {
                ...
                if (isPtrPtr)
                {
                    ...
                    OE_CHECK(_DestroyStructs(fti->sti, ptr, count, dealloc));

                    dealloc(ptr);
                    *(void**)arg = NULL;
                }
                else
                    OE_CHECK(_DestroyStructs(fti->sti, arg, count, dealloc));
            }
            else
            {
                if (isPtrPtr)
    ...
    else if (fti->flags & OE_FLAG_ARRAY)
    {
        if (fti->flags & OE_FLAG_STRING)
        {
            if (fti->type == OE_CHAR_T)
            {
                memset(arg, 0, (size_t)fti->subscript * sizeof(char));
            }
            else
            {
                OE_RAISE(OE_UNEXPECTED);
        ...
        {
            OE_CHECK(_DestroyStructs(
                fti->sti, arg, (size_t)fti->subscript, dealloc));
        }
        else
        {
            memset(arg, 0, _GetTypeSize(fti) * (size_t)fti->subscript);
        }
    }
    else /* scalar */
    {
        if (fti->type == OE_STRUCT_T)
            OE_CHECK(_DestroyStructs(fti->sti, arg, 1, dealloc));
        else
            memset(arg, 0, fti->size);
    }
    ...
    oe_dealloc_proc_t dealloc)
{
    ...
    OE_CHECK(oe_clear_arg(sti, strct, index, isPtrPtr, arg, dealloc));
    ...
    void*(alloc)(size_t size))
{
    ...
    else if (fti->flags & OE_FLAG_PTR)
    {
        ...
        else if (fti->flags & OE_FLAG_COUNT)
        {
            ...
            {
                if (fti->type == OE_STRUCT_T)
                    OE_CHECK(_CloneStructs(fti->sti, fp, count, app, alloc));
                else
                {
                    size_t size = _GetTypeSize(fti);
                    OE_CHECK(_CloneBlob(fp, size * count, app, alloc));
                }
            }
            else
            {
                if (fti->type == OE_STRUCT_T)
                    OE_CHECK(_CopyStructs(fti->sti, fp, count, arg, alloc));
                else
                {
                    size_t size = _GetTypeSize(fti);
    ...
    {
        size_t count = (size_t)fti->subscript;
        ...
        if (fti->flags & OE_FLAG_STRING)
        {
            ...
            OE_CHECK(_CopyStructs(fti->sti, field, count, arg, alloc));
        else
            memcpy(arg, field, fti->size);
    }
    else /* scalar */
    {
        ...
        if (fti->type == OE_STRUCT_T)
            OE_CHECK(_CopyStructs(fti->sti, field, 1, arg, alloc));
        else
            memcpy(arg, field, fti->size);
    }
    ...
    void*(alloc)(size_t size))
{
    ...
    OE_CHECK(oe_set_arg(sti, strct, index, isPtrPtr, arg, alloc));
...
oe_result_t oe_check_pre_constraints(const oe_struct_ti_t* sti, const void* sin)
{
    ...
    for (i = 0; i < sti->nfields; i++)
    {
        ...
        if (!(fti->flags & OE_FLAG_OPT))
        {
            ...
            if (i != 0)
                OE_CHECK(_CheckOptConstraint(sti, sin, fti));
        }
    }
 ...
 */
        ...
        if (fti->type == OE_STRUCT_T)
        {
            ...
            if (fti->flags & OE_FLAG_COUNT)
                OE_CHECK(_GetCount(sti, sin, fti, &n));
            ...
            while (n--)
            {
                OE_CHECK(_TestOrFillPadding(fti->sti, p, test, byte));
                p += fti->sti->size;
            }
        }
#endif
...
oe_result_t oe_check_struct(const oe_struct_ti_t* ti, void* strct)
{
    ...
    OE_CHECK(oe_test_struct_padding(ti, strct));

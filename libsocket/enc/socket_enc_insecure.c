    ...
    size_t a_uiBufferLength)
{
    ...
    if (result.error == 0) {
        oe_strncpy_s(a_pBuffer, a_uiBufferLength, result.name, a_uiBufferLength);
    }
    ...
    _Inout_ socklen_t *optlen)
{
    ...
    if ((oe_result != OE_OK) || (result.len > *optlen)) {
        ...
        *optlen = result.len;
        oe_memcpy_s(optval, (size_t) result.len, result.buffer, (size_t) result.len);
    }
    oe_wsa_set_last_error(OE_NETWORK_INSECURE, (int) result.error);
    return (result.error == 0) ? 0 : OE_SOCKET_ERROR;
}
    ...
    _Inout_ int *a_pAddrLen)
{
    ...
    if ((oe_result != OE_OK) || (result.addrlen > addrlen)) {
        ...
        oe_memcpy_s(a_SockAddr, (size_t) result.addrlen, result.addr, (size_t) result.addrlen);
    ...
    _Inout_ int *addrlen)
{
    ...
    if ((oe_result != OE_OK) || (result.addrlen > *addrlen)) {
        ...
        *addrlen = 0;
        ...
        oe_memcpy_s(addr, (size_t) result.addrlen, result.addr, (size_t) result.addrlen);
        *addrlen = result.addrlen;
    }
    oe_wsa_set_last_error(OE_NETWORK_INSECURE, (int) result.error);
    return (result.error == 0) ? 0 : OE_SOCKET_ERROR;
    ...
    _Out_ oe_addrinfo** ppResult)
{
    ...
    *ppResult = NULL;
    ...
    for (int i = 0; i < count; i++) {
        ...
        oe_memcpy_s(ai->ai_addr, (size_t) response[i].ai_addrlen, response[i].ai_addr, (size_t) response[i].ai_addrlen);

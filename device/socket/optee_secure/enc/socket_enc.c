...
static int fill_internal_fd_set(
    ...
    _Inout_ const oe_socket_provider_t** provider)
{
    ...
    {
        oe_internal_socket_t* internal_socket =
            (oe_internal_socket_t*)fds->fd_array[i];
        ...
        {
            /* We currently only support select() among sockets from the same
             * provider. */
            return -1;
        }
...
static void set_fd_set_results(
...
int oe_shutdown(_In_ oe_socket_t s, int how)
{
    oe_internal_socket_t* internal_socket = (oe_internal_socket_t*)s;
    return internal_socket->provider->s_shutdown(
        internal_socket->provider_socket, how);
}
...
int oe_closesocket(_In_ oe_socket_t s)
{
    oe_internal_socket_t* internal_socket = (oe_internal_socket_t*)s;
    int result =
        internal_socket->provider->s_close(internal_socket->provider_socket);
    free(internal_socket);
    return result;
}

int oe_listen(_In_ oe_socket_t s, int backlog)
{
    oe_internal_socket_t* internal_socket = (oe_internal_socket_t*)s;
    return internal_socket->provider->s_listen(
        internal_socket->provider_socket, backlog);
}
...
{
    oe_internal_socket_t* internal_socket = (oe_internal_socket_t*)s;
    return internal_socket->provider->s_getsockopt(
        internal_socket->provider_socket, level, optname, optval, optlen);
}
...
{
    oe_internal_socket_t* internal_socket = (oe_internal_socket_t*)s;
    return internal_socket->provider->s_recv(
        internal_socket->provider_socket, buf, len, flags);
}
...
{
    oe_internal_socket_t* internal_socket = (oe_internal_socket_t*)s;
    return internal_socket->provider->s_send(
        internal_socket->provider_socket, buf, len, flags);
}
...
static uint32_t swap_uint32(uint32_t const net)
{
    ...
    return (uint32_t)(
        ((uint32_t)data[3] << 0) | ((uint32_t)data[2] << 8) |
...
{
    oe_internal_socket_t* internal_socket = (oe_internal_socket_t*)s;
    return internal_socket->provider->s_setsockopt(
        internal_socket->provider_socket, level, optname, optval, optlen);
}

int oe_ioctlsocket(_In_ oe_socket_t s, long cmd, _Inout_ u_long* argp)
{
    oe_internal_socket_t* internal_socket = (oe_internal_socket_t*)s;
    return internal_socket->provider->s_ioctl(
        internal_socket->provider_socket, cmd, argp);
}
...
{
    oe_internal_socket_t* internal_socket = (oe_internal_socket_t*)s;
    return internal_socket->provider->s_connect(
        internal_socket->provider_socket, name, namelen);
}
...
oe_socket_t oe_accept(
    ...
    _Inout_ int* addrlen)
{
    oe_internal_socket_t* new_internal_socket =
        (oe_internal_socket_t*)malloc(sizeof(*new_internal_socket));
    if (new_internal_socket == NULL)
    {
        return OE_INVALID_SOCKET;
    }
    oe_internal_socket_t* internal_socket = (oe_internal_socket_t*)s;
    intptr_t new_provider_socket = internal_socket->provider->s_accept(
        internal_socket->provider_socket, addr, addrlen);
...
{
    oe_internal_socket_t* internal_socket = (oe_internal_socket_t*)s;
    return internal_socket->provider->s_getpeername(
        internal_socket->provider_socket, addr, addrlen);
}
...
{
    oe_internal_socket_t* internal_socket = (oe_internal_socket_t*)s;
    return internal_socket->provider->s_getsockname(
        internal_socket->provider_socket, addr, addrlen);
}
...
{
    oe_internal_socket_t* internal_socket = (oe_internal_socket_t*)s;
    return internal_socket->provider->s_bind(
        internal_socket->provider_socket, addr, addrlen);
}

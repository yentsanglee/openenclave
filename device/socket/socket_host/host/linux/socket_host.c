    ...
    _Out_ size_t* length_needed)
{
    ...
    *length_needed = count * sizeof(*aibuffer);
    if ((socklen_t)len < *length_needed)
    {
        freeaddrinfo(ailist);
        return EAI_AGAIN;
    }
    ...
    socklen_t a_nOptLen)
{
    int fd = (int)a_hSocket;
    int err =
        setsockopt(fd, (int)a_nLevel, (int)a_nOptName, a_OptVal, a_nOptLen);
    ...
    const oe_fd_set_internal* orig)
{
    ...
    {
        dest->fd_array[i] =
            FD_ISSET((int)orig->fd_array[i], src) ? orig->fd_array[i] : -1;
    }
...
accept_Result ocall_accept(intptr_t a_hSocket, socklen_t a_nAddrLen)
{
    ...
    result.hNewSocket = (intptr_t)accept(
        fd,

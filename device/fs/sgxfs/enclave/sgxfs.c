...
int oe_register_sgxfs_device(void)
{
    ...
    if (oe_allocate_devid(OE_DEVICE_ID_ENCRYPTED_FS) !=
        OE_DEVICE_ID_ENCRYPTED_FS)
        goto done;

    /* Add the sgxfs device to the device table. */
    if (oe_set_devid_device(OE_DEVICE_ID_ENCRYPTED_FS, oe_fs_get_sgxfs()) != 0)
        goto done;

    /* Check that the above operation was successful. */
    if (oe_get_devid_device(OE_DEVICE_ID_ENCRYPTED_FS) != oe_fs_get_sgxfs())
        goto done;

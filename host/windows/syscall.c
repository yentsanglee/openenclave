...
//
char* oe_win_path_to_syscall(const char* path)
{
...
WCHAR* oe_syscall_path_to_win(const char* path, const char* post)
{
    ...
    if (post)
    {
        ...
        if (postlen == 0)
        {
            fprintf(stderr, "oe_syscall_path_to_win string conversion failed winerr=%x\n", GetLastError());
            return NULL;
        }
    }
...
static short _poll_events_to_syscall(short events, short revents)
{
    short ret = 0;
    ...
    int timeout)
{
    ...
    {
        host_fds[i].revents =
            _poll_events_to_syscall(fds[i].events, fds[i].revents);
    }

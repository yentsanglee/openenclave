...
open Ast
open Base.Poly
(* TODO: Remove polymorphic comparison operators. *)
open Plugin
open Printf
open Util
...
let get_array_dims (ns : int list) =
  ...
  if ns = [] then ""
  else Caml.List.fold_left (fun acc n -> acc ^ get_dim n) "" ns
...
let conv_array_to_ptr (pd : pdecl) : pdecl =
  ...
  let get_count_attr ilist =
    ...
    ANumber (Caml.List.fold_left (fun acc i -> acc * i) 1 ilist)
  in
  match pt with
  | PTVal _ -> (pt, declr)
  | PTPtr (aty, pa) ->
...
let get_deepcopy a structs ep =
  let get_struct = function
    | Ptr a -> (
      match a with
      | Foreign n | Struct n -> Caml.List.assoc_opt n structs
      | _ -> None )
    | _ -> None
  in
  ...
  if ep.experimental then
    match s with
    | Some s -> Caml.List.filter should_deepcopy_member s
    | None -> []
  else []
...
let oe_get_param_size (ptype, decl, argstruct) =
  ...
  let attr_value_to_string av =
    match av with
    | None -> ""
    | Some (ANumber n) -> Int.to_string n
  ...
  let decl_size_to_string (ptype : parameter_type) (d : declarator) =
    let dims = List.map d.array_dims (fun i -> "[" ^ Int.to_string i ^ "]") in
...
let oe_get_param_count (ptype, decl, argstruct) =
  ...
  in
  let decl_size_to_string (ptype : parameter_type) (d : declarator) =
    let dims = List.map d.array_dims Int.to_string in
...
let oe_gen_wrapper_prototype (fd : func_decl) (is_ecall : bool) =
  ...
  sprintf "oe_result_t %s(%s)" fd.fname
    ...
    else if List.length args = 1 then List.hd_exn args
    else "\n    " ^ String.concat ~sep:",\n    " args )
...
let emit_composite_type =
  let emit_struct (s : struct_def) =
    ...
    ; "{"
    ; String.concat ~sep:"\n"
        (List.map s.smlist (fun (ptype, decl) ->
             let dims = List.map decl.array_dims (sprintf "[%d]") in
  ...
  let emit_union (u : union_def) =
    ...
    ; "{"
    ; String.concat ~sep:"\n"
        (List.map u.umlist (fun (atype, decl) ->
             let dims = List.map decl.array_dims (sprintf "[%d]") in
             let dims_str = String.concat dims in
  ...
  let emit_enum (e : enum_def) =
    ...
    ; "{"
    ; String.concat ~sep:",\n"
        (List.map e.enbody (fun (name, value) ->
             sprintf "    %s%s" name
               ( match value with
               | EnumVal (AString s) -> " = " ^ s
               | EnumVal (ANumber n) -> " = " ^ Int.to_string n
               | EnumValNone -> "" ) ))
    ; sprintf "} %s;" e.enname
    ; "" ]
  in
...
let get_structs l =
  List.filter_map l (function
    | StructDef s -> Some (s.sname, s.smlist)
    | _ -> None )
...
let oe_gen_args_header (ec : enclave_content) (dir : string) =
  let oe_gen_marshal_struct (fd : func_decl) (errno : bool) =
    ...
    let members =
      ...
      ; List.concat_map (List.map fd.plist conv_array_to_ptr) gen_member_decl
      ]
      |> List.concat
    in
  ...
  let oe_gen_user_includes (includes : string list) =
    if includes <> [] then List.map includes (sprintf "#include \"%s\"")
    else ["/* There were no user includes. */"]
  in
  let oe_gen_user_types (cts : composite_type list) =
    if cts <> [] then List.concat_map cts emit_composite_type
    else ["/* There were no user defined types. */"; ""]
  in
  let oe_gen_ecall_marshal_structs (tfs : trusted_func list) =
    if tfs <> [] then
      List.concat_map tfs (fun tf -> oe_gen_marshal_struct tf.tf_fdecl false)
    else ["/* There were no ecalls. */"; ""]
  in
  let oe_gen_ocall_marshal_structs (ufs : untrusted_func list) =
    if ufs <> [] then
      List.concat_map ufs (fun uf ->
          oe_gen_marshal_struct uf.uf_fdecl uf.uf_propagate_errno )
    else ["/* There were no ocalls. */"; ""]
  in
  let with_errno =
    List.exists ec.ufunc_decls (fun uf -> uf.uf_propagate_errno)
  in
  let guard_macro =
  ...
  Caml.Printf.fprintf os "%s" (String.concat ~sep:"\n" content) ;
  close_out os
...
let oe_prepare_input_buffer (fd : func_decl) (alloc_func : string) structs ep =
  let oe_compute_buffer_size (buffer : string)
    ...
    in
    let params =
      List.concat_map (List.filter plist predicate) (gen_add_size "" "1")
    in
    (* Note that the indentation for the first line is applied by the
     parent function. *)
    if params <> [] then String.concat ~sep:"\n    " params
  ...
  let oe_serialize_buffer_inputs (plist : pdecl list) =
    ...
    let params =
      List.concat_map
        (List.filter plist (fun (p, _) -> is_in_ptr p || is_inout_ptr p))
        (gen_serialize "" "1")
    in
    (* Note that the indentation for the first line is applied by the
     parent function. *)
...
let oe_process_output_buffer (fd : func_decl) =
  ...
  ; (* This does not use String.concat because the elements are multiple lines. *)
    ...
    List.concat_map fd.plist (fun (ptype, decl) ->
...
let oe_gen_call_user_function (fd : func_decl) =
  ...
  ; (match fd.rtype with Void -> "" | _ -> "pargs_out->_retval = ")
    ^ fd.fname ^ "("
  ; String.concat ~sep:",\n    "
      (List.map fd.plist (fun (ptype, decl) ->
...
let oe_gen_out_and_inout_setters (plist : pdecl list) structs ep =
  let params =
    List.concat_map
      (List.filter plist (fun (p, _) -> is_out_ptr p || is_inout_ptr p))
      (oe_gen_set_pointers "" "1" structs ep (fun p ->
...
let gen_fill_marshal_struct (fd : func_decl) structs ep =
  ...
  let rec gen_assignment prefix count (ptype, decl) =
    ...
    |> List.concat
  in
  List.concat_map fd.plist (gen_assignment "" "1")
...
let uses_type (root_type : atype) (fd : func_decl) =
  let param_match =
    List.exists fd.plist (fun (pt, decl) -> root_type = get_param_atype pt)
  in
...
let warn_signed_size_or_count_types (fd : func_decl) =
  ...
  let size_params =
    List.filter_map fd.plist (fun (ptype, _) ->
...
let warn_size_and_count_params (fd : func_decl) =
  let print_size_and_count_warning {ps_size; ps_count} =
    ...
    | _ -> ()
  in
  List.iter fd.plist (fun (ptype, _) ->
      match ptype with
      | PTPtr (_, ptr_attr) when ptr_attr.pa_chkptr ->
...
let validate_oe_support (ec : enclave_content) (ep : edger8r_params) =
  ...
  List.iter ec.tfunc_decls (fun f ->
      ...
      if f.tf_is_switchless then
        failwithf
          "Function '%s': switchless ecalls and ocalls are not yet supported \
           by Open Enclave SDK."
          f.tf_fdecl.fname ) ;
  List.iter ec.ufunc_decls (fun f ->
  ...
  (* Map warning functions over trusted and untrusted function
     declarations *)
  let ufuncs = List.map ec.ufunc_decls (fun f -> f.uf_fdecl) in
  let tfuncs = List.map ec.tfunc_decls (fun f -> f.tf_fdecl) in
  let funcs = List.append ufuncs tfuncs in
  List.iter funcs (fun f ->
...
let gen_t_h (ec : enclave_content) (ep : edger8r_params) =
  let oe_gen_tfunc_prototypes (tfs : trusted_func list) =
    if tfs <> [] then
      List.map tfs (fun f -> sprintf "%s;" (oe_gen_prototype f.tf_fdecl))
    else ["/* There were no ecalls. */"]
  in
  let oe_gen_ufunc_wrapper_prototypes (ufs : untrusted_func list) =
    if ufs <> [] then
      List.map ufs (fun f ->
          sprintf "%s;" (oe_gen_wrapper_prototype f.uf_fdecl false) )
    else ["/* There were no ocalls. */"]
  in
...
let gen_t_c (ec : enclave_content) (ep : edger8r_params) =
  ...
  let oe_gen_ecall_functions =
    if tfs <> [] then List.concat_map tfs (oe_gen_ecall_function structs ep)
    else ["/* There were no ecalls. */"]
  in
...
let gen_u_h (ec : enclave_content) (ep : edger8r_params) =
  let oe_gen_tfunc_wrapper_prototypes (tfs : trusted_func list) =
    if tfs <> [] then
      List.map tfs (fun f ->
          sprintf "%s;" (oe_gen_wrapper_prototype f.tf_fdecl true) )
    else ["/* There were no ecalls. */"]
  in
  let oe_gen_ufunc_prototypes (ufs : untrusted_func list) =
    if ufs <> [] then
      List.map ufs (fun f -> sprintf "%s;" (oe_gen_prototype f.uf_fdecl))
    else ["/* There were no ocalls. */"]
  in
...
let gen_u_c (ec : enclave_content) (ep : edger8r_params) =
  ...
  let oe_gen_host_ecall_wrappers =
    if tfs <> [] then
      List.concat_map tfs
        (oe_gen_host_ecall_wrapper ec.enclave_name structs ep)
    else ["/* There were no ecalls. */"]
  in
  let oe_gen_ocall_functions =
    if ufs <> [] then List.concat_map ufs (oe_gen_ocall_function structs ep)
    else ["/* There were no ocalls. */"]
  in
  let oe_gen_ocall_table =
    ...
    ; "    "
      ^ String.concat ~sep:"\n    "
          (List.map ufs (fun f ->
               sprintf "(oe_ocall_func_t) ocall_%s," f.uf_fdecl.fname ))
    ; "    NULL"
    ; "};" ]
  in
  let content =
...
let gen_enclave_code (ec : enclave_content) (ep : edger8r_params) =
  ...
  Caml.Printf.printf "Success.\n"

(** Install the plugin. *)
let _ =
  Caml.Printf.printf "Generating edge routines for the Open Enclave SDK.\n" ;
